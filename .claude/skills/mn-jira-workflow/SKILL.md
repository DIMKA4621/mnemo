...
- **Description** — the ticket's current living state.
  Overwrite it (`editJiraIssue`) at every meaningful transition.
  Always reflects "what's true about this ticket right now."
- **Comments** (`addCommentToJiraIssue`) — an append-only chronological log.
