...
**Third lesson, root cause of why local testing missed it at all:** `pytest tests/test_install_windows.py` reported "3 passed" before this push and that was read as "the installer tests are green" — false confidence. Those 3 are unrelated unit tests (`test_heartbeat_stall_detection`, `test_heartbeat_zero_tick_edge_case`, `test_disk_space_check`); pytest only collects functions literally named `test_*`, and the actual installer smoke test — the one with the src-shadowing check, venv creation, launcher build, everything — lives in a plain `main()` (line 301) that pytest never touches at all. It only runs via `python tests/test_install_windows.py` directly (`if __name__ == "__main__": raise SystemExit(main())`), which is exactly and only how the CI step invokes it ("Run native Windows installer smoke test"). This dual-mode pattern (a couple of real `def test_*` functions plus a much larger `main()` meant to be run as a script, both in the same file) is a deliberate, repo-wide convention — at least 16 files under `tests/` have their own `def main()`, not just this one — so "pytest says N passed" is systematically insufficient evidence for any of them. Before claiming a file's checks are green, run it BOTH ways: `pytest <file>` for whatever `test_*` functions exist, and `python <file>` directly for whatever `main()` does — matching whatever CI's own step actually invokes, not assuming one implies the other.
