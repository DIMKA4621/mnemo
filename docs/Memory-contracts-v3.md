...
```
```sh
# mcp-setup.sh — три рядки в ЄДИНИЙ виклик sed, перед "$TEMPLATE"
  -e "s|{{MNEMO_HOST}}|${MNEMO_HOST}|g" \
...
сервісі не міг це перевизначити, не правлячи сам шаблон. Хост був просто тією
позицією, яку ще ніхто не міняв.

Наслідок для вже підключених шаблонних проєктів: `MNEMO_HOST` дописує
**звичайний `init`** — разом із рядком `sed`. Це не порушує «звичайний запуск
нічого не видаляє»: він додає. І це обовʼязково, а не охайність — плейсхолдер
без своєї змінної й без рядка підстановки проходить у згенерований
`.mcp.json` дослівно, а скрипт усе одно виходить з 0.
