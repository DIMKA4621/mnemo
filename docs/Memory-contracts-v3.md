...
* Коментар у `src/embedder.py` стверджує: «Measured: peak 5407 MB →
  ...
  *Дія (рішення тимліда, уточнено після фази 0):* пік RSS міряється **разом із
  перезаміром throughput — на реальному банку у фазі 6**, а не на малій фікстурі;
  engine-dev оновлює коментар фактичним числом. Замір фази 0 дав по throughput
  **≈×1.5 (12 CPU, мала фікстура)** — напрям підтверджено, NFR-5 закрито, але
  точнішої цифри не фіксуємо: обидва способи виміру ганяли ту саму фікстуру крізь
  того самого резидента, тож збіг був арифметичний, а не незалежний. Інакше в коді
  лишається **точна цифра, яка стала неправдою** — гірше, ніж відсутність цифри. Це
  також входить у NFR-9 («одна копія моделі ~1.6 ГБ»): якщо пік поїде вгору, поїде
  й вимога.
