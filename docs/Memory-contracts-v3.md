...
Окремий **роут** (`/settings`, `frontend/src/app/settings/page.tsx`), а не модалка: решта діалогів — про один банк і висять над роботою, цей — про машину і роботу заміщає.
Відкривається постійним пунктом бічної навігації (`Sidebar.tsx`) з гвинтиком-іконкою, не шестернею в топбарі, яка відкривала оверлей у vanilla-кабінеті.
...
Екран — окремий Next.js-роут (`frontend/src/app/settings/page.tsx`), а розділи — горизонтальні вкладки AntD `Tabs` під шапкою сторінки (`frontend/src/components/settings/SettingsTabs.tsx`), що замінили колишній лівий rail з вертикальною навігацією.
Масив вкладок у `SettingsTabs.tsx` разом із ключами `settings.tabs.*`/`settings.lede.*` у файлах локалізації (`uk.json`/`en.json`) — єдине джерело назв, порядку й підписів; кожна вкладка монтує свій компонент-розділ (`GeneralSection`, `EmbedSection`, `MaintenanceSection`).
Рішення про кнопку «Зберегти» лежить не в таблиці, а в самому розділі: змонтований розділ реєструє `SectionController` (`{hasPendingChange, busy, submit}`) через пропс `onController`, і `SettingsTabs` показує кнопку у футері, лише поки контролер не `null` — розділ без незбережуваних полів (`MaintenanceSection`) просто ніколи його не реєструє, замість того щоб мати `submit: null` у спільній таблиці.

| розділ | що там | зберігає |
|---|---|---|
| Загальні | `GeneralSection` — автозапуск, автооновлення, обов'язковий логін, далі статус служби | так |
| Модель ембедингу | §6.6.1 — `EmbedSection`: бекенд, модель, ендпоінт | так |
| Обслуговування | `MaintenanceSection` — structured `doctor`, явний `clean-orphans` | ні |
