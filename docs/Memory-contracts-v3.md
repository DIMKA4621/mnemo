...
def index_file(
    ...
    *,
    ...
    Returns the number of chunks written, or the batch index to resume from
    if `should_yield()` asked us to step aside (preemption, section 8.3)."""
...
* Виняток — **preemption** (8.3): там `index_file` виходить *керовано*,
  повертаючи номер батчу, і `files` так само лишається незаписаним, тому
  зовнішня консистентність та сама.
...
2. **`name`** — точний збіг імені. Імена унікальні (6.5), тож форма
   однозначна.
3. **Шлях** — абсолютний або `~`-шлях. Спершу точний збіг кореня; інакше —
   **найглибший** банк, чий корінь є предком цього шляху (щоб `cwd` сесії
   Claude Code резолвився у свій банк). Нічого не знайдено → `BankNotFound`.

Ця третя форма — те, чим користується auto-inject хук: він знає лише `cwd`.

`AmbiguousBankRef` лишається в API як **захисний випадок**: реєстр —
людиночитний файл, і руками в нього можна вписати два однакові імені.
Тоді `resolve` не вгадує, а піднімає помилку (`bank_ambiguous`, 409).
Через `add` / `update` таке не проходить.
...
**Токен.** `STATE_DIR / "api.token"`, 48 hex-символів, створюється при першому
старті, `chmod 0600` (best-effort) — точно як `embed.token`.
...
* **Без токена:** `/health` (потрібен `service_ctl`, щоб перевірити живість до
  ...
  Усе під `/api` і `/mcp` — під токеном. **[NEW]**
...
| `index_start` | воркер узяв задачу | `{"task_id","kind","path","batches","trigger"}` |
...
Три рівні, у порядку спадання пріоритету:

1. Аргумент тула `bank` (id / назва / шлях) — **завжди працює**, це запасний
   шлях, якщо заголовок не доїде.
2. Заголовок запиту **`X-Mnemo-Bank: <name>`** з `.mcp.json` — це і є
   «підключення вказує, з яким банком працює» (design §6): проєкт або агент
   тримає **своє MCP-підключення на свій банк**. У заголовку стоїть
   **назва**, не `bank_id` — див. 10.4.
3. Якщо в реєстрі рівно **один** банк — він і береться.

Нічого з цього не спрацювало → тул повертає текст з переліком доступних банків
і просить вказати `bank`, а не кидає помилку (агент має що робити далі).

> **Ризик реалізації, перевірити на фазі 4:** доступ до HTTP-заголовків із тіла
> тула залежить від версії `mcp` SDK. Якщо заголовок недоступний — п.1 і п.3
> лишаються робочими, а `.mcp.json` має класти `bank` інакше (напр. окремий
> шлях монтування). Це запис для service-dev, не для мовчазного обходу.
...
```json
{
  "mcpServers": {
    "mnemo": {
      "type": "http",
      "url": "http://127.0.0.1:8918/mcp",
      "headers": {
        "Authorization": "Bearer ${MNEMO_API_TOKEN}",
        "X-Mnemo-Bank": "odin-crm"
      }
    }
  }
}
...
**Жодного машинозалежного значення у git-трекованому файлі** — це рішення
тимліда й правило, що переживає v3:

* Токен: `.mcp.json` підставляє `${MNEMO_API_TOKEN}`, а саму змінну експортує
  інсталятор (PowerShell-профіль на Windows, shell-профіль на Linux — зона
  platform-dev, NFR-6). **[NEW]**
* Банк: `X-Mnemo-Bank` несе **назву** банку (`"odin-crm"`), а не
  `bank_id`. `bank_id` — це `sha1(абсолютний шлях)[:16]`, тобто значення,
  правильне рівно на одній машині з одним шляхом чекауту; після `git clone`
  в іншу теку воно вказує в нікуди. Назва стабільна, і саме тому реєстр
  тримає її унікальною (6.5). **[NEW]**
* Кілька банків = кілька записів у `mcpServers` (`mnemo`, `mnemo-notes`, …) —
  саме так design §2 розводить ізоляцію.

Якщо назви немає в реєстрі цієї машини (клонував колега, банк ще не
зареєстровано), тул повертає текст «банк `odin-crm` не зареєстровано; доступні:
…» — агент бачить, що робити, і сесія не падає.
...
| `mnemo init [--root]` | **локальна** | вирівнює `.mcp.json` / `.claude/` проєкту |
...
| `mnemo doctor` | **локальна** | діагностика: venv, deps, модель, токени, порти, банки **[NEW]** |
| `mnemo hook-inject` | **локальна обгортка** | читає hook-JSON, шле `POST /api/search` |
...
Стан процесу — `STATE_DIR / "service.json"`, який пише сам бекенд на старті й
знімає на виході: **[NEW]**
...
Спавн — windowless, без винятків (NFR-1): Windows —
`CREATE_NO_WINDOW | DETACHED_PROCESS` + `pythonw.exe`; POSIX —
`start_new_session=True` + stdio→devnull. Це стосується **і** бекенда, **і**
embed-резидента.

> **Іменований must-fix фази 5 (рішення тимліда).** `embed_server._spawn_server`
> (`src/embed_server.py:164`) зараз ставить лише
> `DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP`. Правила v3 прямо кажуть, що
> самого `DETACHED_PROCESS` **не досить** — консоль блимає. Це не «прибирання
> за нагоди», а окремий пункт фази 5 з власною перевіркою: «жодного вікна не
> блимає» стосується і резидента, який автостартує з клієнта. Виправлення в
> `embed_server.py` (файл engine-dev) координує тимлід — platform-dev не
> редагує його самовільно.
...
| Об'єкт | `--migrate` |
|---|---|
| `mcpServers.mnemo` у формі v2 (`/bin/sh -c … mnemo mcp`) | переписує на HTTP-форму 10.4 |
...
| `MNEMO_EMBED_IDLE_TIMEOUT` | `0` **(змінено)** | daemon / engine-dev | v3 тримає модель теплою постійно (design §4) **[NEW]** |
| `MNEMO_EMBED_THREADS` | `cpu*3//4` | daemon / engine-dev | стеля ONNX-потоків (NFR-5, фаза 0) |
| `MNEMO_EMBED_POOL` | `1` | daemon / engine-dev | к-сть інстансів резидента (фаза 7) |
...
Чотири питання, які документ виносив на розвилку, **закриті**. Записано тут,
щоб їх не переоткривали.

| # | Питання | Рішення | Де в тексті |
|---|---|---|---|
| 1 | Адреса банку в git-трекованому `.mcp.json` | **назва**, не `bank_id`, не `${MNEMO_BANK_ID}`; реєстр примусово тримає назви унікальними | 6.5, 10.3, 10.4 |
...
| 4 | `test_mcp.py` — спавнить `mnemo mcp` по stdio, шле `scope`/`agent` | tester | 0 (частково) / 4 | HTTP-клієнт MCP; scope-асерт видалено |
...
**`tests/test_mcp.py` — зламаний уже зараз, не у фазі 4.** Фаза 0 прибрала
`scope`/`agent` з `memory_search` (перевірено в `src/mcp_server.py`), а тест
на рядках 85–93 їх передає й вимагає ізоляції по `agent-memory/reviewer/`.
Це не «застаріє потім» — воно червоне сьогодні. Далі, у фазі 4, помирає й
решта файлу: він спавнить `mnemo mcp` по stdio через `StdioServerParameters`,
а підкоманди `mcp` не буде.
*Розвʼязаний стан:* тест переписується на HTTP-клієнт MCP проти піднятого
in-process застосунку; асерт ізоляції замінюється на **ізоляцію між двома
банками** (це і є v3-шлях розділення, рішення #13), а не між скоупами.

**`tests/test_platform.py` — проходить, і в цьому пастка.** Три місця:

| Асерт | Зараз | Після фази 4 | Що має асертити |
|---|---|---|---|
| `portable MCP definition` | зелений | **зелений, але порожній** — порівнює `_MCP_SERVER` сам із собою | явну HTTP-форму: `type=="http"`, url на loopback+порт, `Authorization` містить **плейсхолдер** `${MNEMO_API_TOKEN}`, а не літерал, `X-Mnemo-Bank` — **назва**, а не 16-hex |
| `legacy MCP definition is an explicit conflict` | зелений на L1 | **дірка** — L2 не покрита | відмову на **обох** L1 і L2 |
| `MCP idempotent` / `hooks idempotent` | зелений | лишається валідним | без змін |

Асерт «`X-Mnemo-Bank` — не 16-hex» варто написати буквально: це єдиний
машинний regression-guard правила «жодного машинозалежного значення в
git-трекованому файлі» (§10.4), яке інакше легко порушити назад.
...
* Коментар у `src/embedder.py` стверджує: «Measured: peak 5407 MB →
  ...
  `EMBED_THREADS_FRACTION = (3, 4)`). З **вимкненою ареною** кожен
  intra-op-потік алокує транзієнтно, тож пік RSS масштабується з кількістю
  потоків. Число 1563 MB більше не підтверджене.
  *Дія:* tester переміряє пік RSS резидента при новій стелі в тій самій
  перевірці фази 0, де міряє throughput; engine-dev оновлює коментар
  фактичним числом. Інакше в коді лишається **точна цифра, яка стала
  неправдою** — гірше, ніж відсутність цифри. Це також входить у NFR-9
  («одна копія моделі ~1.6 ГБ»): якщо пік поїде вгору, поїде й вимога.
...
| **`.mcp.json` відсутній**; є `.mcp.json.template` з формою **L1** (`/bin/sh`) + `mcp-setup.sh`, який генерує з нього | `mnemo init --migrate` **не побачить** шаблон — він редагує `.mcp.json` | **ні** — шаблон авторства проєкту, за рішенням Q2 mnemo його не переписує; правити руками |
...
Два з цих чотирьох рядків **не покриті нічим із написаного** і потребують
рішення до фази 4:
...
2. **`.mcp.json.template` + `mcp-setup.sh`.** Проєкт генерує свій
   `.mcp.json`, тож ланцюг оновлення проходить повз mnemo. Кому переписати
   шаблон під HTTP-форму — користувачу чи скілу `mnemo-adopt` — вирішує
   тимлід.
