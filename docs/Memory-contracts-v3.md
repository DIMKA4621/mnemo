...
Чотири рівні, у порядку спадання пріоритету (одна функція
`api.resolve_bank_ref` на всі обличчя, а не по одній на кожне):

1. Аргумент тула `bank` (id / назва / шлях) — **завжди працює**, це запасний
   шлях, якщо wiring не доїхав.
2. **Сегмент URL — `/mcp/<name>`.** Це **основний** шлях: він нічого не
   вимагає від клієнта, крім уміння сходити за URL. Саме тут «підключення
   вказує, з яким банком працює» (design §6): проєкт або агент тримає **своє
   MCP-підключення на свій банк**. Назва **percent-encoded** (`safe=""`) —
   людські назви містять пробіли й кирилицю, а сирий пробіл робить URL
   невалідним.
3. Заголовок **`X-Mnemo-Bank: <name>`** — **підтримується як override, але на
   нього не покладаємось**. Причина в 10.4: пересилання `headers` для
   `type: http` залежить від клієнта, і якщо воно не спрацює, то разом з
   адресацією відвалиться й автентифікація.
4. Якщо в реєстрі рівно **один** увімкнений банк — він і береться (це не
   здогадка: вибору немає).
...
> **Поправка після фази 4.** Перша редакція цього розділу ставила заголовок
> вище за URL і тримала окремим ризиком «а раптом SDK не віддасть заголовок».
> Ризик знято зміною порядку: банк і токен їдуть **у URL**, а заголовок
> лишається сумісним запасним варіантом.
...
```json
{
  "mcpServers": {
    "mnemo": {
      "type": "http",
      "url": "http://127.0.0.1:8918/mcp/odin-crm?token=${MNEMO_API_TOKEN}",
      "headers": {
...
**І банк, і токен їдуть у URL** — заголовки лишаються продубльованими, але
жодна з двох речей на них не тримається. Причина одна: пересилання `headers`
для `type: http` — властивість клієнта, і якщо воно не спрацює, зламається не
лише адресація банку, а й автентифікація. Назва банку в шляху —
**percent-encoded** (`safe=""`): пробіли й кирилиця в людських назвах —
норма на цій машині, а сирий пробіл робить URL невалідним.
...
* Банк: і сегмент URL, і `X-Mnemo-Bank` несуть **назву** банку
  (`"odin-crm"`), а не `bank_id`. `bank_id` — це `sha1(абсолютний шлях)[:16]`, тобто значення,
