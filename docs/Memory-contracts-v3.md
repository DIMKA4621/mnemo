...
#### 6.6.1 Сторінка налаштувань у кабінеті **[NEW]**
...
* **Зупинки й перезапуску служби тут немає, і це рішення, не пропуск.**
  ...
  рівно той розрив, який кабінет має закрити. «Перезапустити» має ту саму
...
* **Окремий ендпоінт, не поле в `/api/status`.** Відповідь коштує підпроцес
  (`schtasks`/`systemctl`) — **виміряно ~45 мс**, — а `/api/status` кабінет
...
* Контрол — **`.segmented` «Увімкнено│Вимкнено»**, той самий, що
  ...
  тему кабінету ігнорує — рівно та неоднорідність, заради якої компонент і
  заведено. Правило кабінету лишається: **нових візуальних патернів не
...
(`bge-m3:latest` = `BAAI/bge-m3` = `bge-m3`) — інакше кабінет відмовився б
...
україномовному кабінеті посеред екрана стоїть англійський рядок з API.
...
його в текст, а `GET /api/doctor` віддає ті самі дані кабінету. Парсингу CLI-
...
* **Без токена:** `/health` (потрібен `service_ctl`, щоб перевірити живість до
  ...
  типово **весь `/api`** — кабінет і CLI, лише loopback, без явно
  налаштованого токена (`$MNEMO_API_TOKEN`) авторизація там не вмикається
  взагалі (2026-08-21: логін-токен на локальному кабінеті не купував
