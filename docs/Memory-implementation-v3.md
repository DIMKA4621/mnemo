...
- **Перенесено з фази 0:** перезаміряти на справжньому банку throughput ембедингу при
  стелі `cpu*3//4` — мала фікстура для цієї цифри не показова. Памʼять уже переміряно:
  усталений стан 1503–1526 МБ, пік ~1.56 ГБ (короткі англійські чанки) / ~2.07 ГБ
  (український markdown, `BATCH_SIZE=16`); від стелі потоків пік **не залежить**
  (9 проти 4 потоків — 1.0 МБ різниці). Деталі — NFR-9 і контракти 15.7.
- ✅ **Перевірка:** UI відкривається на loopback; видно банки/дерево/вміст; межі чанків
  ...
  прогрес через WS); журнал фільтрується за банком і глобально; throughput переміряно на
  реальному банку й записано.
