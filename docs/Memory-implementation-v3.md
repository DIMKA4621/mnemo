...
1. **Більше потоків одному демону.** Стеля `EMBED_THREADS = cpu*3//4` (замість `cpu//3`),
   ...
   *Заміряно у фазі 0:* **≈×1.5 на 12-CPU машині, мала фікстура** — усередині
   ...
   — арифметика, а не незалежне підтвердження. **Перезамір — на реальному банку у
   фазі 6**, разом із піком RSS резидента (15.7 контрактів).
...
- **Перенесено з фази 0:** на першому справжньому банку перезаміряти throughput ембедингу
  й **пік RSS резидента** при стелі `cpu*3//4` (розділ 4; контракти 15.7) — мала фікстура
  для цих цифр не показова.
- ✅ **Перевірка:** UI відкривається на loopback; видно банки/дерево/вміст; межі чанків
  ...
  прогрес через WS); журнал фільтрується за банком і глобально; обидва заміри (throughput,
  пік RSS) зроблені на реальному банку й записані.

**Фаза 7 — Опційне масштабування**
- B: `api`-провайдер (`httpx`). A: пул воркерів-демонів (N, default 1) з балансуванням.
- ✅ **Перевірка:** перемикання провайдера → індекс банку перебудовується, пошук працює;
  з пулом `N=2` терміновий запит не чекає на bulk-білд.
...
- **Потоки демону:** стеля `cpu*3//4`, override `MNEMO_EMBED_THREADS`; заміряно **≈×1.5
  (12 CPU, мала фікстура)**, перезамір на реальному банку у фазі 6 разом із піком RSS (розділ 4).
