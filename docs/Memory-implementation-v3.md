...
- ✅ **Перевірка:** UI відкривається на loopback; видно банки/дерево/вміст; межі чанків
  ...
  прогрес через WS); журнал фільтрується за банком і глобально.
...
- **Міграція індексів:** ключ `sha1(root)` лишається, але корінь тепер **корінь банку**, а не
  проєкту → наявні `state/<projhash>.db` не перевикористовуються. Індекс одноразовий і
  відтворюваний, тож міграція = **перебудова з `.md`** (нічого не конвертуємо); старі файли
  прибираються при першому запуску v3.
